# Stage packet — S4_stress_tradeoff_analysis — eth-mainnet-usdat-gearbox-oracle
...
Mandatory facts:
- `oracle.S4_stress_tradeoff_analysis.eth-mainnet-usdat-gearbox-oracle.scope_loaded` — stage scope, input paths, and required outputs are loaded before claims (if unresolved: `not_investigated`)
- `oracle.S4_stress_tradeoff_analysis.eth-mainnet-usdat-gearbox-oracle.f1_borrower-pool-liquidator-side-specific-stress-sc` — borrower/pool/liquidator side-specific stress scenarios (if unresolved: `not_investigated`)
- `oracle.S4_stress_tradeoff_analysis.eth-mainnet-usdat-gearbox-oracle.f2_size-liquidity-staleness-redemption-and-route-as` — size, liquidity, staleness, redemption, and route assumptions (if unresolved: `not_investigated`)
- `oracle.S4_stress_tradeoff_analysis.eth-mainnet-usdat-gearbox-oracle.f3_tradeoff-conclusion-derived-from-fact-states-rat` — tradeoff conclusion derived from fact states rather than placeholder gaps (if unresolved: `not_investigated`)
