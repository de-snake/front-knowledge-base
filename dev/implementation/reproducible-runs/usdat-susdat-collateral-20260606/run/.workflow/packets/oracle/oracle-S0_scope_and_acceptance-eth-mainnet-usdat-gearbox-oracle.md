# Stage packet — S0_scope_and_acceptance — eth-mainnet-usdat-gearbox-oracle
...
Input paths:
- `tokens/eth-mainnet-usdat-gearbox-oracle/scope.json`
...
Mandatory facts:
- `oracle.S0_scope_and_acceptance.eth-mainnet-usdat-gearbox-oracle.scope_loaded` — stage scope, input paths, and required outputs are loaded before claims (if unresolved: `not_investigated`)
- `oracle.S0_scope_and_acceptance.eth-mainnet-usdat-gearbox-oracle.f1_oracle-scope-chain-protocol-market-credit-manage` — oracle scope: chain, protocol, market/credit manager, position side, token role, and position size (if unresolved: `not_investigated`)
- `oracle.S0_scope_and_acceptance.eth-mainnet-usdat-gearbox-oracle.f2_feed-address-or-replayable-feed-discovery-plan` — feed address or replayable feed-discovery plan (if unresolved: `not_investigated`)
- `oracle.S0_scope_and_acceptance.eth-mainnet-usdat-gearbox-oracle.f3_accepted-methodologies-and-non-execution-accepta` — accepted methodologies and non-execution acceptance policy (if unresolved: `not_investigated`)
