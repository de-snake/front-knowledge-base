# Stage packet — S2_node_classification — eth-mainnet-susdat-gearbox-oracle
...
Mandatory facts:
- `oracle.S2_node_classification.eth-mainnet-susdat-gearbox-oracle.scope_loaded` — stage scope, input paths, and required outputs are loaded before claims (if unresolved: `not_investigated`)
- `oracle.S2_node_classification.eth-mainnet-susdat-gearbox-oracle.f1_node-class-and-math-reconstruction-for-each-feed` — node class and math reconstruction for each feed dependency (if unresolved: `not_investigated`)
- `oracle.S2_node_classification.eth-mainnet-susdat-gearbox-oracle.f2_source-primitive-scaling-staleness-and-fallback-` — source primitive, scaling, staleness, and fallback behavior (if unresolved: `not_investigated`)
- `oracle.S2_node_classification.eth-mainnet-susdat-gearbox-oracle.f3_unclassified-nodes-carry-explicit-unknown-state-` — unclassified nodes carry explicit unknown state and evidence path (if unresolved: `not_investigated`)
...
```json
{
  ...
  "proof_id": "oracle.S2_node_classification.<fact>.no_result.v1",
