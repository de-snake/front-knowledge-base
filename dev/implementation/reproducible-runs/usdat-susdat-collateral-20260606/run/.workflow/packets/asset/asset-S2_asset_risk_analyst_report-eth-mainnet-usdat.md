# Stage packet — S2_asset_risk_analyst_report — eth-mainnet-usdat
...
Mandatory facts:
- `asset.S2_asset_risk_analyst_report.eth-mainnet-usdat.scope_loaded` — stage scope, input paths, and required outputs are loaded before claims (if unresolved: `not_investigated`)
- `asset.S2_asset_risk_analyst_report.eth-mainnet-usdat.f1_s1-source-evidence-imported-without-silent-gaps` — S1 source evidence imported without silent gaps (if unresolved: `not_investigated`)
- `asset.S2_asset_risk_analyst_report.eth-mainnet-usdat.f2_risk-classification-by-admin-issuer-backing-tran` — risk classification by admin, issuer/backing, transferability, liquidity, oracle, and governance axis (if unresolved: `not_investigated`)
- `asset.S2_asset_risk_analyst_report.eth-mainnet-usdat.f3_explicit-unresolved-fact-states-and-decision-eff` — explicit unresolved fact states and decision effects (if unresolved: `not_investigated`)
...
- `tokens/eth-mainnet-usdat/analyst-report.md`
- `tokens/eth-mainnet-usdat/verification.md`
