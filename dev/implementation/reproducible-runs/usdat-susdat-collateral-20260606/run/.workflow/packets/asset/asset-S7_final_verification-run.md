# Stage packet — S7_final_verification — run
...
Mandatory facts:
- `asset.S7_final_verification.run.scope_loaded` — stage scope, input paths, and required outputs are loaded before claims (if unresolved: `not_investigated`)
- `asset.S7_final_verification.run.f1_required-reports-and-indexes-exist` — required reports and indexes exist (if unresolved: `not_investigated`)
- `asset.S7_final_verification.run.f2_fact-state-summary-matches-stage-artifacts` — fact-state summary matches stage artifacts (if unresolved: `not_investigated`)
- `asset.S7_final_verification.run.f3_preview-execute-remain-blocked-unless-a-separate` — Preview/Execute remain blocked unless a separate proposal gate is satisfied (if unresolved: `not_investigated`)
...
```json
{
  ...
  "proof_id": "asset.S7_final_verification.<fact>.no_result.v1",
