# Demo run — USDat / sUSDat collateral Analyze → Propose

Generated: 2026-06-06

Purpose: show a completed harness validation path for a real collateral-analysis question without committing bulky generated `tmp/` run artifacts.
...
The run was re-validated from the repository root with:

```bash
python3 dev/tools/run_workflow.py analyze-propose \
  --input dev/implementation/workflow-harness/tmp/inputs/usdat-susdat-collateral-fresh-20260606T133313Z.json \
...
`tmp/` paths are local/generated and intentionally ignored by git.
...
Formal validation passed, but business/proposal readiness stayed conservative:
...
sUSDat remains more conditional because the feed uses ERC-4626 accounting over USDat while immediate recovery can depend on thinner secondary liquidity, queue processing, and issuer/STRC realization.
...
This is the desired behavior for the public harness: deterministic validation can pass the artifact structure and explicit evidence/state envelopes, while the parent workflow still blocks Preview / Execute when live decision inputs are missing.
