...
1. Filters the unified feed by chain, target collateral token, access (`permissionless` / `kycRequired`), and agent-side whitelist.
   ==resolved_note: what if there are 0 strategies available based on token/chain? 
   do we interfere and suggest bridging / swapping?
   > yes, but on Analyze stage and only for bridging (same asset on other chain).
   > Press the button - bridge through {partner}.
   > Please note that we're not responsible for this 3rd party service. ==
...
2. Reads each `StrategyOpportunity` for sizing bounds (==`minDebt`, `maxDebt`==), `borrowableLiquidity`, `maxLeverage`, `borrowApy`, and the headline economics (`maxLeverageYield`, ==`bestBaseYield`==).
   ...
   ==resolved_note: for the minDebt and maxDebt, don't we need to estimate a desired position size from the user first? Different CMs have different minDebt and maxDebt.
   > Use min(CM[minDebt]) and max(CM[maxDebt])==
   
   ==resolved_note: bestBaseYield - why is that interesting to the user?
   >Find a way to aggregate CMs - if base yield / maxLeverage is different for different CMs / curators - display==
...
- Collateral token yield (base APY), current + 90d history — if this is below borrow cost, more leverage = more loss.
  combine w/ borrow rate on the same graph + net + rewards
  ![[Screenshot 2026-04-21 at 15.59.20.png]]
...
- Liquidation fee and premium — how much the user loses beyond position value if liquidated.
  We shouldn't show loss from liquidation - it's scary for the user. 
  Add deleverage bot info if applicable
...
- Entry swap cost at position size — swapping underlying → collateral has real price impact; at moderate sizes ==it can be 2–3 weeks of yield.==
  Show: quotaIncreaseFee, Total fees
...
- ==90d volatility==.
  ...
  ==resolved_note: historical price chart? do we have this data for any given token / oracle?
  e.g. heartbreak + threshold for Chainlink - oracles might lie (inside of threshold range)==
  
  ==note: in general, showing oracle-based price might be a bad idea.
  we do not collect real market price
  in the Collateral details we might show real market price vs oracle price==
...
- adapter-accessible liquidity (the user can only route through CM-approved adapters, not the full market).
  ==note: when any operation happens (e.g. swap during account opening), we use our own router. 
  Our router might provide worse swap offers compared to 1inch / CoWswap, for example. 
  Idea: fetch DEXs and compare to our own router.==
...
- ==historical main and reserve oracle prices (90d daily)
  main for most cases, check main+resolve on partial withdrawal==, x
  
  ==resolved_note: should we provide that? or just a link to the oracle contract?
  > show in advanced / oracle-specific page==
...
- Curator / controller address + name — trust frame.
  ...
  and info about bad dept for example - check other aggregators
  maybe chatgpt with weekly updates about incidents and bad debt==
...
Parameter change log (CM-level): LT reductions (the user's HF could drop again), token forbids (e.g. in the past, rsETH was added. Today we want to disable borrowing against rsETH aka exit routes shrink), IRM changes (borrow rate spike). Pending governance changes queued in Safe TX or timelock — with description, execution time, and affected parameters.

==note: could add our severity position on each point - is it critical or not?==

example:
![[Screenshot 2026-04-21 at 16.33.44.png]]
...
2. What size, at what target leverage? (Constrained by `minDebt` / `maxDebt`, `borrowableLiquidity`, internal concentration cap.)
    Add info about credit manager potential / suggested
