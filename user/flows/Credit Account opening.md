...
**Scope tiers.** Same `T1` / `T2` system as [[Pool deposit#Stage 2 · Analyze — LP due diligence|Pool deposit Stage 2]]. CA-specific drills live in [[Credit Account opening - reference]]; flow-agnostic drills (oracle types, Steakhouse layers, curator pillars) are reused from [[Pool deposit - reference]].
...
| IRM curve sensitivity | T2 | If +10 pp utilisation jump moves borrow rate >X bps, economics fragile — leveraged net APY can flip negative under modest pool-side shifts. Drill goes deeper than the headroom check above. [[Credit Account opening - reference#Drill — IRM curve sensitivity\|drill ↗]] | `Pool.IRM.{U1, U2, Rbase, Rslope1, Rslope2, Rslope3}`; current utilisation; sensitivity computation. |
...
[[Credit Account opening - reference#Drill — Multicall preview mechanics|drill ↗]] covers the SDK router simulation surface and what each check is sensitive to.
...
| Signer context | HITL wallet OR scoped bot signer. **For KYC-gated CMs:** execution uses the compliance-gated path, and bots are blocked — see [[Credit Account opening - reference#Drill — KYC-gated execution path|drill ↗]]. |
