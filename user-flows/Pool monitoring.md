...
| **Social.** | For institutional LPs: produce a defensible monthly check-in artefact ("position is green; here are the green/yellow/red bands; here is the curator change log since last check"). |
...
- **Detect RWA drift** (RWA pools only) — frozen-accounts delta, frozen-debt delta, whitelisted-liquidator changes. _(Q7)_

**Exit gate:** every default Q (Q1–Q5; Q7 if RWA) resolves to green. If any Q flips yellow / red, the session escalates from Confirmation to a focused Analyze re-run on the affected [[Pool deposit#Stage 2 · Analyze — LP due diligence|Pool deposit Q]] (see [[#Back-edge · Analyze (focused re-run)|back-edge below]]). Q6 (oracle, P2) is a drill — it doesn't gate by default; it fires only when triggered (see Q6 framing). Thresholds are the canonical bands in [[Benchmarks and tresholds for metrics#Pool-related|Benchmarks]].
...
| --- | --- | --- | --- |
| Composite APY vs floor + 30d trend | T1 | Verdict per [[Benchmarks and tresholds for metrics#Pool-related\|Benchmarks]]: composite ≥ floor and 30d trend flat or positive = green; > 10 % 30d decline = red. The 30d trend is a property of the pool series, anchored to the rolling window — not to the LP's entry. | `PoolOpportunity.yield.composite.{current, 30dSeries}`; `userThesis.floorApy`. |
...
| Sub-question | Tier | What the agent does | Data retrieved |
| --- | --- | --- | --- |
| Withdrawable now vs my position | T1 | `(1 − U) × TVL` vs `LpPosition.currentValueUsd`. Verdict per [[Benchmarks and tresholds for metrics#Pool-related\|Benchmarks]] utilisation bands (< 80 % green; 80–90 % yellow; > 90 % red). Withdrawal fee surfaced alongside. | `Pool.{availableLiquidity, expectedLiquidity, utilisation, withdrawalFee}`; `LpPosition.currentValueUsd`. |
| Utilisation 30d trend | T1 | Trend vs 30d series. Flat / declining = green; slow rise = yellow; structural rise to new high = red. | `Pool.utilisation.30dSeries`. |
| Cascade- vs trap-shape symptoms on dominant collateral | T2 | Cross-ref the dominant token's current oracle category: NAV / hardcoded → trap symptoms (utilisation pinning without LP-run); market → cascade symptoms (withdrawal-vs-repayment race). [[Pool deposit - reference#Drill — Collateral-induced liquidity risk by oracle type\|drill ↗]] | Current oracle category/methodology for the dominant token; recent withdraw / liquidate / repay event mix. |
...
| Sub-question | Tier | What the agent does | Data retrieved |
| --- | --- | --- | --- |
| Top-3 collateral delta since last check | T1 | Compare current top-3 quoted-token exposure (by `quotaUsed`) to the agent's last-check snapshot. Verdict per [[Benchmarks and tresholds for metrics#Pool-related\|Benchmarks]]: same top-3 = green; new top-3 entrant = yellow; dominant collateral changed = red. Drift triggers the forward-looking re-eval — "would the LP accept this composition entering today?" | `Pool.quotedTokens[].quotaUsed` (current); `agentLog.previousCheck.quotaComposition`. |
...
**Exit gate:** "Pending governance queue is empty or non-material; recent parameter-change log is quiet (≤ 1 material change in 30d) per [[Benchmarks and tresholds for metrics#Pool-related|Benchmarks]]."
...
| Sub-question | Tier | What the agent does | Data retrieved |
| --- | --- | --- | --- |
| Pending governance queue | T1 | Read pending Safe-TX / timelock items affecting this pool. Each item: classification (material vs cosmetic) + execution time. Material change queued within monitoring window = red. | pending governance-change queue filtered to this pool. |
| Recent parameter-change log | T1 | Material changes executed since the last monitoring check. Quiet (≤ 1 / quarter) = green; moderate = yellow; frequent = red. | Executed pool-change feed since the last monitoring check. |
...
| Sub-question | Tier | What the agent does | Data retrieved |
| --- | --- | --- | --- |
| Share price delta since last check | T1 | Any drop = canary trigger (per [[Benchmarks and tresholds for metrics#Pool-related\|Benchmarks]] any sudden drop = red; minor decline < 0.1 % = yellow). Distinct from yield accrual, which is reflected in monotonic share-price growth. | `Pool.sharePrice` (current); `agentLog.previousCheck.sharePrice`; pool's known-incidents log if any. |
| Insurance fund balance delta | T1 | ==insurance-fund balance change (only relevant if the buffer was part of the original thesis)==. Stable / growing = green; minor decline = yellow; material decline or approaching zero = red. | `Pool.insuranceFundBalance` (current); `agentLog.previousCheck.insuranceFundBalance`. |
...
**Why this matters.** Oracle manipulation / staleness is a **P2** LP loss vector ([[Personas and audience#Pool LP (passive lender)|Personas]]) — upstream cause of bad-debt and locked-liquidity events when it fires. Q6 fires only when triggered — see [[Pool monitoring - reference#Drill — Q6 oracle drill triggers|drill ↗]]. Deep-dive in [[Pool deposit#Q2 · What's my maximum exposure, per token?|Pool deposit Q2]]. The methodology check splits into two layers: (i) detect change via the agent log; (ii) re-evaluate acceptability under the current thesis.

**What the agent computes (when Q6 fires):**

| Sub-question | Tier | What the agent does | Data retrieved |
| --- | --- | --- | --- |
| Per-token freshness | T2 | `now − lastUpdate < stalenessWindow` for each quoted token. One token approaching the window = yellow; past window = red. Cheapest sub-Q — runs first when Q6 fires. | Oracle last-update timestamp and staleness window per quoted token. |
| Main-vs-reserve divergence | T2 | Compare `mainOracle` and `reserveOracle` prices per token. Small divergence = green; moderate = yellow; large = red. | Main/reserve oracle prices per quoted token. |
...
### Q7 · Is the compliance layer drifting? *(RWA pools only)*

**Exit gate:** "Frozen-accounts count stable; aggregate frozen debt well below insurance fund; whitelist and liquidator set unchanged or expanded (never contracted)."

**Why this matters.** Frozen accounts and liquidator scarcity is the RWA-specific LP loss vector ([[Personas and audience#Pool LP (passive lender)|Personas]] — P2). When the issuer (Securitize, etc.) freezes accounts, those positions can't be liquidated; if their aggregate debt exceeds the insurance fund, the loss socialises to LPs. Deep-dive: [[Pool deposit#Q2 · What's my maximum exposure, per token?|Pool deposit Q2]] Per-token risk profile (Platform layer for issuer-controlled tokens).
...
| Sub-question | Tier | What the agent does | Data retrieved |
| --- | --- | --- | --- |
| Frozen-accounts delta | T1 | Count of frozen accounts new since `agentLog.previousCheck.asOf` + their aggregate debt. Stable or zero = green; slow rise = yellow; total debt approaching insurance fund = red. | RWA-platform-specific endpoint (Securitize, etc.): `frozenAccounts[].{address, frozenAt, debtUsd}`; `agentLog.previousCheck.frozenAccountIds`. ==note: endpoint not specified — flag to [[Data requirements and to-dos]].== |
| Whitelist / liquidator changes | T2 | ==whitelist changes for liquidators.== Liquidator set contracted (fewer entities can clear bad accounts) = red; expanded = info-only. | RWA-platform whitelist endpoint with change history. |
...
MonitoringSnapshot {
  position_id, as_of,
  verdicts: { q1_yield, q2_exit, q3_composition, q4_governance, q5_bad_debt, q6_oracle?, q7_rwa? },
  // each verdict: { color: 'green'|'yellow'|'red', summary, contributing_signals[] }
  // q6_oracle is present only when Q6 fired (drill triggered); absent on default Confirmation calls
  // q7_rwa is present only for RWA pools
  drift_signals: string[],   // human-readable summary of any non-green verdicts
...
**Hand-off branches.**
- **All green** → `recommended_next: 'end'`. Agent surfaces a single summary line and exits — Confirmation session, in under a minute.
- **Any non-green** → `recommended_next: 'analyze'`. Hand off to [[#Stage 2 · Analyze (Pool) — focused re-run|Stage 2]] for the focused Pool-deposit-Q re-run.
...
For each non-green Q-verdict in `MonitoringSnapshot.verdicts`, re-run the mapped Pool-deposit Q. Deep-dives live in [[Pool deposit#Stage 2 · Analyze — LP due diligence|Pool deposit Stage 2]] and are not duplicated here.
...
| Q7 RWA drift | [[Pool deposit#Q2 · What's my maximum exposure, per token?\|Q2]] | Per-token 3-layer risk profile, Platform-layer pillar specifically. |
| **Synthesis** | — | Targeted Q re-run, not full Analyze — no fresh Discover, no re-traversal of Qs that were green in Stage 6. Each re-run Q produces a "still acceptable" or "no longer acceptable" verdict; aggregated into a position-level `position_thesis_verdict`. | — |
...
| **Synthesis** | — | Output `ActionDecision { action_class, amount_usd, rationale }`. Action-class mapping, no-Emergency rationale, and cross-pool-reallocation out-of-scope rule: [[Pool monitoring - reference#Drill — LP action-class palette\|drill ↗]]. | — |
...
| **Utilisation spike lockout.** | LP tries to exit; utilisation is at 98 %; withdrawal is throttled until borrowers repay. Caught by **Q2** (red verdict on withdrawable + 30d trend). |
...
| **RWA freeze cascade.** | Issuer freezes multiple accounts. Their total debt exceeds the insurance fund. Socialised loss hits LPs. Caught by **Q7** (T1 frozen-accounts delta vs insurance fund) and signalled through **Q5** (share-price drop) once realised.  |
